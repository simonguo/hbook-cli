...
function resolveRemoteVersion(version) {
    ...
    return availableVersions()
        .then(function (available) {
            ...
            // Check version
            if (!version) throw "Invalid version or tag '" + _version + "', see available using 'hbook versions:available'";
            return version;
        });
}
...
// Install a specific version of hbook
function installVersion(version, forceInstall) {
    return resolveRemoteVersion(version)
        ...
        .spread(function (tmpDir) {
            ...
            console.log("Installing HBook", version);
            return Q.nfcall(npmi.bind(npmi), options).thenResolve(tmpDir);
        })
        .then(function (tmpDir) {
            var gitbookRoot = path.resolve(tmpDir, "node_modules/hbook");
            ...
            if (!tags.isValid(version)) throw "Invalid HBook version, should satisfies " + config.GITBOOK_VERSION;
...
function uninstallVersion(version) {
    if (!version) return Q.reject(new Error("Need a version of HBook to uninstall"));
    var outputFolder = path.resolve(config.VERSIONS_ROOT, version);
...
function getVersion(version) {
    return getCurrentVersion(version)
        .then(function (resolved) {
            ...
            if (!gitbook) throw "HBook Version " + resolved.tag + " is corrupted";
            return gitbook;
        });
}
